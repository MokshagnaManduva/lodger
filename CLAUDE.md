...
Not built: `packtool generate` / `preview`, the hand-finished canonical sprite, motion and
locomotion, perching, discrete socket rotation (`orientFrames`), and the app shell
(menu bar, settings, pack manager, Sparkle, signing).
...
**Next, in order:** motion and locomotion (walking, falling, dragging) → the app shell. In parallel: hand-finish the canonical Klien sprite from
